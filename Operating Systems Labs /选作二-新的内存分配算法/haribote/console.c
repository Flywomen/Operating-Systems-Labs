...
void cons_newline(struct CONSOLE *cons)
{
	...
	if (cons->cur_y < 28 + 272) {
		...
		if (sheet != 0) {
			...
			sheet_refresh(sheet, 8, 28, 8 + 240, 28 + 288);
		}
	}
	cons->cur_x = 8;
	return;
...
int *hrb_api(int edi, int esi, int ebp, int esp, int ebx, int edx, int ecx, int eax)
{
	...
	if (edx == 1) {
	...
	} else if (edx == 21) { // FirstFit
		MEM_ALLOC_METHOD = 0;
		cons_putstr0(cons, "First Fit\n");
	} else if (edx == 22) { // BestFit
		MEM_ALLOC_METHOD = 1;
		cons_putstr0(cons, "Best Fit\n");
	} else if (edx == 23) { // WorstFit
		MEM_ALLOC_METHOD = 2;
		cons_putstr0(cons, "Worst Fit\n");
	} else if (edx == 28) { // printFreeMem
		switch (MEM_ALLOC_METHOD) //显示内存分配算法
		{
			case 0:
				cons_putstr0(cons, "First Fit\n");
				break;
			...
			case 2:
				...
				break;
			default:
				cons_putstr0(cons, "Error Alloc Method!\n");
				break;
		}
		//显示当前空闲块情况
		...
		{
			char s[60] = {0};
			sprintf(s,"Addr:%08x Size:%8d\n", memman->free[i].addr, memman->free[i].size);
			cons_putstr0(cons, s);
		}
	}
	return 0;
...
//LDT
		/*char tmp[32] = {0};
		sprintf(tmp,"Code LDT:0x%02x%02x%04x\n",task->ldt[1].base_high,task->ldt[1].base_mid,(task->ldt[1].base_low)&0xff);
		cons_putstr0(cons,tmp);
		//ds_base
		memset(tmp, 0, 32);
		sprintf(tmp,"DS_BASE:0x%08x\n",task->ds_base);
		cons_putstr0(cons,tmp);*/
