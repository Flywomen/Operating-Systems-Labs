...
struct SHEET *open_console(struct SHTCTL *shtctl, unsigned int memtotal)
{
	...
	unsigned char *buf = (unsigned char *) memman_alloc_4k(memman, 256 * 325);
	sheet_setbuf(sht, buf, 256, 325, -1); /* 透明色なし */
	make_window8(buf, 256, 325, "console", 0);
	make_textbox8(sht, 8, 28, 240, 288, COL8_000000);
...
void close_console(struct SHEET *sht)
{
	...
	memman_free_4k(memman, (int) sht->buf, 256 * 325);
	sheet_free(sht);
	close_constask(task);
	return;
}
