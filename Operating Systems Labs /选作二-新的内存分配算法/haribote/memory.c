...
/* 確保 */
{
	unsigned int i, a, addr, tar, maxOff, minOff;
	int mark = 0;
	switch(MEM_ALLOC_METHOD)
	{
		...
		case 1://Best-fit
		{
			...
			for (i = 0; i < man->frees; ++i)
			{
				...
				else if (man->free[i].size > size)//寻找相差最小的空闲块
				{
					if (mark == 0)//确定minOff的初始值
					{
						minOff = man->free[i].size - size;
						a = i;
						mark = 1;
					}
					else 
					{
						if (man->free[i].size - size < minOff)
						{
							minOff = man->free[i].size - size;
							a = i;
						}
