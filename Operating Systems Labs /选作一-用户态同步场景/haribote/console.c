...
void set_time_waiting(int time){
	char timerbuf[10];
	struct TIMER * qiqi = timer_alloc();
	struct FIF032 * fifo;
	fifo32_init(fifo, 10, timerbuf, task_now());
	timer_init(qiqi, fifo, "1");
	timer_settime(qiqi, time);
	for(;;){
...
void task_b_main(struct SHEET *sht_win_b,struct TASK * task)
{
	...
	struct SHAR * shar_b = shar_alloc("sharedint",0);
	//shar_b->shared=0;
...
void task_c_main(struct SHEET *sht_win_b,struct TASK * task)
{
	...
	//���z����?��
	struct SHAR * shar_c = shar_alloc("sharedint",0);
	//shar_c.shared=0;
...
void jingzheng(){
	...
	struct SHEET *sht_win_b[2];
	unsigned char * buf_win_b;
	struct TASK *task_b[2];
	char s[40];
	int i=0;
	...
	for (i = 0; i < 2; i++) {
		...
		*((int *) (task_b[i]->tss.esp+8)) = (int)task_b[i];
		task_run(task_b[i],1,3);
	}
	sheet_slide(sht_win_b[0], 400,  200);
	sheet_slide(sht_win_b[1],   600, 200);
	sht_win_b[0]->task=task_b[0];
	sht_win_b[1]->task=task_b[1];
...
int *hrb_api(int edi, int esi, int ebp, int esp, int ebx, int edx, int ecx, int eax)
{
	...
	//else if (edx == 26) {
	//	reg[7] = shar_alloc((char *) ebx + ds_base);
	//}
	else if (edx == 27) {
		jingzheng();
	}
	//struct SHAR * api_sharealloc(char *name,int mode);
		...
		reg[7] = shar_alloc((char *) ebx + ds_base,ecx);
	}
	//struct SHAR * api_get(struct SHAR *shar, int position);
	else if(edx==29){
		reg[7] = get((struct SHAR *)ebx,ecx);
	}
	//change(struct SHAR *shar, int position, char ch);
	else if(edx==30){
		change((struct SHAR *)ebx,ecx,esi);
	}
	else if(edx==31)
	{
		reg[7] = getin((struct SHAR *)ebx);
	}
	else if(edx==32)
	{
		reg[7] = getout((struct SHAR *)ebx);
	}
	else if(edx==33)
	{
		changein((struct SHAR *)ebx,ecx);
	}
	else if(edx==34)
	{
		changeout((struct SHAR *)ebx,ecx);
	}
	else if(edx==35)
	{
		set_time_waiting(ebx);
	}
	return 0;
